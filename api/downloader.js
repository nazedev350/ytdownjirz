    ...
    const poll = async () => {
      try {
        ...
        if (res.progress >= 1000) {
          resolve({ title: data.title, downloadUrl: res.download_url, thumbnail: data.info?.image });
  ...
  for (let i = 0; i < 40; i++) {
    try {
      ...
      if (res.success && res.download_url) {
        return { title: data.info?.title, downloadUrl: res.download_url, thumbnail: data.info?.image };
      }
    } catch {}
    await delay(2500);
  }
...
async function quaternaryDownload(url, type = "mp3") {
  ...
  throw new Error("SSYoutube hanya mendukung MP3.");
}

async function ytmp3(url) {
  try { return await primaryMP3(url); } catch (e1) {
    console.error("[MP3] Primary gagal:", e1.message);
    try { return await secondaryDownload(url, "mp3"); } catch (e2) {
      console.error("[MP3] Secondary gagal:", e2.message);
      try { return await tertiaryDownload(url, "mp3"); } catch (e3) {
        console.error("[MP3] Tertiary gagal:", e3.message);
        try { return await quaternaryDownload(url, "mp3"); } catch (e4) {
          console.error("[MP3] Quaternary gagal:", e4.message);
...
async function ytmp4(url, quality = "720") {
  try { return await primaryMP4(url, quality); } catch (e1) {
    console.error("[MP4] Primary gagal:", e1.message);
    try { return await secondaryDownload(url, "mp4", quality); } catch (e2) {
      console.error("[MP4] Secondary gagal:", e2.message);
      try { return await tertiaryDownload(url, "mp4"); } catch (e3) {
        console.error("[MP4] Tertiary gagal:", e3.message);
...
module.exports = { ytmp3, ytmp4, extractVideoId };
